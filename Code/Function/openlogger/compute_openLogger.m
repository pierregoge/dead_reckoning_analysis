function [data_tag, data,duration] = compute_openLogger(srate,limit_analyse_1,limit_analyse_2,seconds_offset_openlogger,plot_raw_data_logger,debug_signal_analyzer)
...
data_raw = openlogger_load(); % load data file
...
data = timetable(time_analyse,data.gyro(length_d,1:3),data.accel(length_d,1:3),data.mag(length_d,1:3),data.depth(length_d,1),data.spin(length_d,1),'VariableNames',varNames);
