function [diff_speed,diff_cumul_speed, diff_eul_yaw,diff_cumul_eul_yaw] = err_speed_yaw_ref2(data_heading,data_speed,limit_analyse,offset_heading)

yaw(:,1) = data_heading.trigo(:,1)+offset_heading;
yaw(:,2) = data_heading.gyro(:,1)+offset_heading;
yaw_ref = (data_heading.ref2)/180*pi;
...
for i=2:length(yaw(limit_analyse,1))
...
%     end
% end
diff_cumul_eul_yaw = zeros(length(yaw_ref(limit_analyse,1)),length(yaw(1,1:end)));
diff_eul_yaw = zeros(length(yaw_ref(limit_analyse,1)),length(yaw(1,1:end)));
...
diff_speed = zeros(length(limit_analyse),3);

diff_speed(limit_analyse,1) = data_speed.ref(limit_analyse,1)-data_speed.sensor(limit_analyse,1);
diff_speed(limit_analyse,2) = data_speed.ref(limit_analyse,1)-data_speed.acc(limit_analyse,1);
diff_speed(limit_analyse,3) = data_speed.ref(limit_analyse,1)-data_speed.acc_mag(limit_analyse,1);

diff_cumul_speed = zeros(length(limit_analyse),3);
