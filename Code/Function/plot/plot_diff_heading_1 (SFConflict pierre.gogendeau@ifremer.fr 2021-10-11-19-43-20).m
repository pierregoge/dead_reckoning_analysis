...
subplot(2, 2, 2)
plot_offset = 6500;
plot((limit_traj(1)+plot_offset:limit_traj(end))/10,abs(diff_eul_yaw(plot_offset:end-1,2)),'r')
hold on
plot((limit_traj(1)+plot_offset:limit_traj(end))/10,abs(diff_eul_yaw(plot_offset:end-1,1)),'b')
...
xlabel('Time (s)')
ylabel('Error heading (°)')
hold off

subplot(2, 2, 4)
yyaxis left
abs(smoothdata(odba(limit_traj(1)+plot_offset:limit_traj(end)),'movmean',100))
plot((limit_traj(1)+plot_offset:limit_traj(end))/100,abs(smoothdata(odba(limit_traj(1)+plot_offset:limit_traj(end)),'movmean',100)))
hold on
yyaxis right
plot((limit_traj(1)+plot_offset:limit_traj(end))/100,speed_100(limit_traj(1)+plot_offset:limit_traj(end)))
% hold on
% plot((limit_traj(1):limit_traj(end))/100,data_speed.ref(limit_traj(1):limit_traj(end)))
hold off
xlim([(limit_traj(1)+plot_offset)/100 limit_traj(end)/100])
ylim([0 2])
