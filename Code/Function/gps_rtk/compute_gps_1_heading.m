function [gps_ref,gps_ref_a] = compute_gps_1_heading(gps,gps_ref_1)
...
gps.lon_gps_1 = smoothdata(gps.lon,'movmean','SmoothingFactor',0.001);

%gps.lat_gps_2 = smoothdata(gps.lat_gps_2,'movmean','SmoothingFactor',0.001);
gps.lat_gps_1 = smoothdata(gps.lat,'movmean','SmoothingFactor',0.001);
...
angle2 = angle2-90;
for i=1:length(angle2(:,1))
    if (angle2(i,1) < -180)
        angle2(i,1) = angle2(i,1)+(2*180);
    elseif  (angle2(i,1) > 180)
        angle2(i,1) = angle2(i,1)-(2*180);
    end
end
            
varNames = {'x','y','x_f','y_f','heading','lat','lon','lat_f','lon_f','heading2'};
gps_ref = timetable(gps.time_gps_1,x,y,x_f,y_f,angle2(:,1),lat,lon,lat_f,lon_f,angle2,'VariableNames',varNames);

length_dur = 1:size(gps_ref.Time);
duration = seconds(length_dur/10);

varNames = {'heading','heading2'};
gps_ref_a = timetable(duration',gps_ref.heading2,gps_ref.heading2,'VariableNames',varNames);
